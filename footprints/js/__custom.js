 ...
 */
ajaxFn =
        function ($, formid, routes, editor_field) {
            $(formid).bootstrapValidator({
                ...
                submitHandler: function (validator, form, submitButton) {
                    ...
                    if (editor_field != undefined) {
                        data.push({name: editor_field, value: CKEDITOR.instances.wysiwyg.getData()});
                    }
        ...
        function ($, formid, routes, editorfield) {
            ...
            $(formid).bootstrapValidator({
                ...
                submitHandler: function (validator, formid) {
                    var url = baseurl + routes;
                    var form = $(formid);
                    if (editorfield != undefined) {
                        var formData = new FormData($(form)[0]);
                        var data1 = CKEDITOR.instances.wysiwyg.getData();
                        formData.append(editorfield, data1);
                    } else {
                        var formData = new FormData($(form)[0]);
                    }
                    $.ajax({
